...
/*
	Version 0.2 Imago
	- Compiles /w VC9
	- Renamed asynchttp.h for SVN
	- NYI: multiple sockets/states
	- TODO: OpenSSL for future server-player connections
...
Connection::Connection( const char* host, int port ) :
	...
	m_UserData(0),
	m_State( IDLE ),	// Imagoing to need an array of these
	m_Host( host ),
	m_Port( port ),
	m_Sock(-1)			// Imagoing to need an array of these
{
}
...
void Connection::connect()
{
	...
	m_Sock = socket( AF_INET, SOCK_STREAM, 0 );  //Imagoing to support more than one socket!
	if( m_Sock < 0 )
