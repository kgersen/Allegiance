...
CPigEngine::CPigEngine() :
  ...
  m_eLobbyMode(PigLobbyMode_Club)
{
}
...
void CPigEngine::ProcessScriptDirChanges()
{
  ...
  WIN32_FIND_DATA ffd;
  TCFileFindHandle hff = FindFirstFile(m_bstrScriptDir + "*.pig", &ffd);
  ...
  for (it = m_mapScripts.begin(); it != m_mapScripts.end(); ++it)
  {
...
HRESULT CPigEngine::put_ScriptDir(BSTR bstrScriptDir)
{
  ...
  long lr = key.SetValue(bstrValue, TEXT("ScriptDir"));
  if (ERROR_SUCCESS != lr)
    return HRESULT_FROM_WIN32(lr);
...
HRESULT CPigEngine::put_ArtPath(BSTR bstrArtPath)
{
  ...
  long lr = key.SetValue(bstrValue, TEXT("ArtPath"));
  if (ERROR_SUCCESS != lr)
    return HRESULT_FROM_WIN32(lr);
