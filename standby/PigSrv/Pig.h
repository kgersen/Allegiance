...
const MAX_USER_PASSWORD_LENGTH = 10;
...
public:
  ...
  virtual void    OnPostCreate(FedMessaging * pthis, IDirectPlayX* pdpIn, IDirectPlayX** pdpOut);
...
template<class T>
findMaxThresholdExceeded(const T* pItems, int cItems, const T& value)
{
  T   tMax =  0;
  int iMax = -1;
